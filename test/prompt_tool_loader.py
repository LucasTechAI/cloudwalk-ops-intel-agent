...
class TestPromptLoader(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.patcher: Any = patch(
            "src.agents.utils.prompt_tool_loader.PATH_ROOT", Path(self.test_dir)
        )
        self.patcher.start()
    ...
    def test_format_prompt(self) -> None:
        ...
        formatted: str = self.loader.format(
            "test_prompt.txt", name="Alice", place="Wonderland"
        )
...
class TestToolsLoader(TestCase):
    ...
    def test_get_tool_from_list(self) -> None:
        ...
        tool: dict[str, object] | None = self.loader.get_tool(
            "test_tools.json", "search_tool"
        )
...
class TestIntegration(TestCase):
    ...
    def test_full_workflow(self) -> None:
        ...
        loader: AgentResourceLoader = AgentResourceLoader(
            prompts_dir=self.prompts_dir, tools_dir=self.tools_dir
        )

        prompt: str = loader.format_prompt(
            "workflow.txt", agent_name="TestAgent", task="analyze data"
        )
...
class TestResultCollector(TextTestResult):
    ...
    def addSuccess(self, test) -> None:
        ...
        self.test_results.append({"test": test.id(), "status": "✓ PASS", "message": ""})

    def addError(self, test, err):
        """Records a test that resulted in an error"""
        super().addError(test, err)
        self.test_results.append(
            {"test": test.id(), "status": "✗ ERROR", "message": str(err[1])}
        )
    ...
    def addFailure(self, test, err):
        ...
        self.test_results.append(
            {"test": test.id(), "status": "✗ FAIL", "message": str(err[1])}
        )
    ...
    def addSkip(self, test, reason):
        ...
        self.test_results.append(
            {"test": test.id(), "status": "⊘ SKIP", "message": reason}
        )
...
def print_test_report(result) -> None:
    ...
    for class_name, tests in sorted(results_by_class.items()):
        ...
        logger.info(f"\n📦 {class_name}")
        logger.info(
            f"   Tests: {class_total} | Successes: {class_passed}/{class_total}"
        )
        logger.info("   " + "-" * 76)
